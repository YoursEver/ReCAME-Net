...
import os
class tsne_function_CVPR(object):
    ...
    def tsne_pca(self):
        self.img = np.concatenate(self.tsne_img)
        self.label = np.concatenate(self.tsne_label)
        self.tsne = TSNE(n_components=2, init='pca').fit_transform(self.img)

    def visualize_4shot(self, output_path, g):
        ...
        for cls in range(self.num_classes):
            #print(cls)
            if cls in g[0]:
                this_colors = colors[0]
            elif cls in g[1]:
                this_colors = colors[1]
            elif cls in g[2]:
                this_colors = colors[2]
            else:
                this_colors = colors[3]

            idx = np.where(self.label==cls)
            idx = idx[0] # 原先idx=(array([ 3, 10, 21, 27, 44, 52]), dtype)
                        # print(colors[cls])
                        #print(cls)
                    
                        #plt.scatter(tsne[idx, 0], tsne[idx, 1], label=self.id2cat[str(cls)], c=colors[cls])
            plt.scatter(self.tsne[idx, 0], self.tsne[idx, 1], c=this_colors, marker="o")
        ...
        plt.savefig('./Train Tsne of feat 4 shot.png')
        #plt.savefig('./Tsne of head shot.png')
        
        plt.close()

    def visualize_4head(self, output_path, g):
        # print('Total Img shape: {}, Label shape: {}'.format(np.array(self.tsne_img).shape, np.array(self.tsne_label).shape))
        ...
        for cls in range(self.num_classes):
            ...
            if cls in g[0]:
                ...
                idx = idx[0] # 原先idx=(array([ 3, 10, 21, 27, 44, 52]), dtype)
                            ...
                            #plt.scatter(tsne[idx, 0], tsne[idx, 1], label=self.id2cat[str(cls)], c=colors[cls])
                plt.scatter(self.tsne[idx, 0], self.tsne[idx, 1], c=colors[count], marker="o")
                count += 1
        ...
        plt.ylim(-100, 100)
        #plt.legend(loc='best')
        #output_path = os.path.join(config.OUTPUT_DIR, config.NAME, '{}.png'.format('Tsne of feat 4 head'))
        plt.savefig('./Train Tsne of feat 4 head.png')
        #plt.savefig('./Tsne of head shot.png')
        plt.close()
