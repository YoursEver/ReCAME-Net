...
class IMBALANCED_WHITEBAIT(Dataset):
    def __init__(self, dataset, loader=default_loader):
        if dataset == '28k':
            # path = './datasets/RawData_28k_rename/'    # dataset path
        ...
        for i in range(len(num_per_cls)): #看n個類別各自的數量
            data_x = data_path[data_sum:(data_sum+num_per_cls[i])]
            data_y = data_label[data_sum:(data_sum+num_per_cls[i])]
            
            tv_img, test_img, tv_label, test_label = train_test_split(data_x, data_y, random_state=777, test_size=1/6) # 切train, test
            train_img, valid_img, train_label, valid_label = train_test_split(tv_img, tv_label, random_state=777, test_size=1/5) # 從train再切train, val
